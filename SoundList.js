
var soundsList = [
	{
		name:"Walking on a leaves",
		tag:["outdoor", "leaves", 'forest', "autumn", "nature", "walking", "walk"]
	},
	{
		name:"Lightina a match",
		tag:["human", "fire", 'match', "matches", "lighting", "smoke", "smoking"]
	},	
	{
		name:"Leaves",
		tag:["outdoor", "leaves", 'forest', "autumn", "nature"]
	},
	{
		name:"Leaves falling",
		tag:["outdoor", "leaves", 'forest', "autumn", "nature"]
	},
	{
		name:"Breaking",
		tag:["breaking", "glass", 'splinter', "crashing", "break"]
	},
	{
		name:"Breaking glass",
		tag:["breaking", "glass", 'splinter', "crashing", "break"]
	},
	{
		name:"Breaking branches",
		tag:["breaking", "snapping", 'popping', "break", "snap", 'pop']
	},
	{
		name:"roar2",
		tag:["animal", "roar", 'roaring']
	},
	{
		name:"bed movment",
		tag:['house', "bed", "cracking", "crunching", 'popping','crack', 'crunch', 'pop']
	},
	{
		name:"roar",
		tag:["animal", "roar", 'roaring']
	},
	{
		name:"door knocking",
		tag:["house", "knocking",'knock']
	},
	{
		name:"breaking wood",
		tag:["breaking", "snapping", 'popping', "break", "snap", 'pop']
	},
	{
		name:"breathing",
		tag:['human',"breathing", "panting", 'inhaling', 'exhaling', 'gasping', 'respiring', "breath", "pant", 'inhale', 'exhale', 'gasp', 'respire']
	},
	{
		name:"crunch",
		tag:['house','human', "crunch", "crunching"]
	},
	{
		name:"cracking 3",
		tag:['house', "cracking", "crunching", 'popping', "crack", "crunch", 'pop']
	},
	{
		name:"cracking",
		tag:['house', "cracking", "crunching", 'popping', "crack", "crunch", 'pop']
	},
	{
		name:"dropping heavy 4",
		tag:['house', "outdoor", "dropping", 'dripping', 'drops', "drop", 'drip', 'drops']
	},
	{
		name:"dropping heavy 6",
		tag:['house', "outdoor", "dropping", 'dripping', 'drops', "drop", 'drip', 'drops']
	},
	{
		name:"dropping heavy 7",
		tag:['house', "outdoor", "dropping", 'dripping', 'drops', "drop", 'drip', 'drops']
	},
	{
		name:"dropping heavy 8",
		tag:['house', "outdoor", "dropping", 'dripping', 'drops', "drop", 'drip', 'drops']
	},
	{
		name:"dropping heavy",
		tag:['house', "outdoor", "dropping", 'dripping', 'drops', "drop", 'drip', 'drops']
	},
	{
		name:"squirting",
		tag:['human', "house", "squirting", 'squeeze', "squirt", 'squeezing']
	},
	{
		name:"squirting 2",
		tag:['human', "house", "squirting", 'squeeze', "squirt", 'squeezing']
	},
	{
		name:"squirting heavy",
		tag:['human', "house", "squirting", 'squeeze', "squirt", 'squeezing']
	},
	{
		name:"door knocking 2",
		tag:["house", "knocking", 'door knocking', 'hitting', "knock", 'door knock', 'hit']
	},
	{
		name:"jolting",
		tag:["house", "jolting", 'knocking over', 'convulse', "jolt", 'knock over', 'convulsing']
	},
	{
		name:"jolting 2",
		tag:["house", "jolting", 'knocking over', 'convulse', "jolt", 'knock over', 'convulsing']
	},
	{
		name:"splashing",
		tag:["human", 'outdoor', "splashing", 'spilling', "splash", 'spill']
	},
	{
		name:"man moaning",
		tag:["human", 'moaning', 'moan']
	},
	{
		name:"squelch",
		tag:["outdoor", 'squelch', 'squelching']
	},
	{
		name:"squelch 2",
		tag:["outdoor", 'squelch', 'squelching']
	},
	{
		name:"squelch 3",
		tag:["outdoor", 'squelch', 'squelching']
	},
	{
		name:"sipping",
		tag:["human", 'sipping', 'slurping', 'sip', 'slurp']
	},
	{
		name:"noise",
		tag:["electric", 'electronic', 'noise']
	},
	{
		name:"tearing",
		tag:["human", 'tearing', 'ripping off']
	},
	{
		name:"rolling",
		tag:["house", 'rolling', 'spinning', 'rettling', 'roll', 'spin', 'rettle']
	},
	{
		name:"steps female",
		tag:["human", 'steps', 'walking', 'walk']
	},
	{
		name:"steps female 2",
		tag:["human", 'steps', 'walking', 'walk']
	},
	{
		name:"steps male",
		tag:["human", 'steps', 'walking', 'walk']
	},
	{
		name:"steps male 2",
		tag:["human", 'steps', 'walking', 'walk']
	},
	{
		name:"crunch 2",
		tag:['house','human', "crunch", "crunching"]
	},
	{
		name:"crunch 3",
		tag:['house','human', "crunch", "crunching"]
	},
	{
		name:"crunch 4",
		tag:['house','human', "crunch", "crunching"]
	},
	{
		name:"crunch 5",
		tag:['house','human', "crunch", "crunching"]
	},
	{
		name:"swallowing",
		tag:['human', "swallowing", 'swallow']
	},
	{
		name:"tearing 2",
		tag:["human", 'tearing', 'ripping off']
	},
	{
		name:"wet sound",
		tag:["outdoor", 'wet', 'squelch', 'squelching']
	},
	{
		name:"wet sound 2",
		tag:["outdoor", 'wet', 'squelch', 'squelching']
	},
	{
		name:"zipper",
		tag:["human", 'zipper']
	} 
];
